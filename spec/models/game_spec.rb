...
RSpec.describe Game, type: :model do
  ...
  describe 'game#stalemate' do
    ...
    it 'returns true when game is a stalemate' do
      ...
      Piece.destroy_all
      King.create(x_cord: 5, y_cord: 7, game_id: g.id, color: 'white')
      King.create(x_cord: 5, y_cord: 5, game_id: g.id, color: 'black')
      Pawn.create(x_cord: 5, y_cord: 6, game_id: g.id, color: 'white')
      expect(g.stalemate?).to eq true
    end
  ...
  describe '#check?' do
    ...
    it 'should return true for check if king is put in check by bishop' do
      g = Game.create(id: 1)
      Piece.destroy_all
      g.update(current_turn: 'black')
      king = King.create(color: 'black', x_cord: 5, y_cord: 7, game_id: g.id)
      Pawn.create(color: 'white', x_cord: 5, y_cord: 6, game_id: g.id)
      Bishop.create(color: 'white', x_cord: 0, y_cord: 2, game_id: g.id)
      expect(g.check?).to eq true
    end

    it 'should return true if king is put in check by pawn' do
      g = Game.create(id: 1)
      Piece.destroy_all
      g.update(current_turn: 'black')
      king = King.create(color: 'black', x_cord: 5, y_cord: 7, game_id: g.id)
      Pawn.create(color: 'white', x_cord: 5, y_cord: 6, game_id: g.id)
      Pawn.create(color: 'white', x_cord: 4, y_cord: 6, game_id: g.id)
      expect(g.check?).to eq true
    end

    it 'should return true if king is put in check by pawns' do
      g = Game.create(id: 1)
      Piece.destroy_all
      g.update(current_turn: 'black')
      king = King.create(color: 'black', x_cord: 5, y_cord: 7, game_id: g.id)
      Pawn.create(color: 'white', x_cord: 5, y_cord: 6, game_id: g.id)
      Pawn.create(color: 'white', x_cord: 4, y_cord: 6, game_id: g.id)
      Pawn.create(color: 'white', x_cord: 6, y_cord: 6, game_id: g.id)
      expect(g.check?).to eq true
    end
